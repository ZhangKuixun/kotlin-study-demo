...
/**
 * 分析：
 * 执行到value1，value1是一个async异步函数，线程进入value1的函数里，让协程等待20秒，线程跳出
 * value1；接着执行到value2，线程进入value2的函数里，线程先睡2秒，然后抛出异常，此时value1还在
 * 等待；2秒后value2抛出异常，value1也不等待了，直接进入finally，调用函数的地方也会进入finally。
 *
 * 小结：
 * 1.子协程取消，会沿着协程的层次关系向上传播，直到父协程及调用父协程的代码也取消或者抛出异常。
 * 2.挂起函数中的有两个协程，自己取消，兄弟协程也会取消。
 */