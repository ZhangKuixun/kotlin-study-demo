 ...
 *
 * 中间操作的运算符：map、filter、take、zip，不是挂起函数，应用于上游流的函数，并返回下游流。中间操作符不执行流中的任何代码，只是建立了一个执行操作链，然后迅速返回。这就是冷流特性。
 *
 * 中止操作运算符：collect、single、reduce、toList...，或是launchIn：在给定域中启动流的收集。是挂起函数，触发所有流的操作。
 *
 * Flow的构建器：
 * flowOf：从一组固定的值创建一个流。
 * asFlow：扩展函数，将各种类型转换为流。
 * flow：调用emit构造任意流。
