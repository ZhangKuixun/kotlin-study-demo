 ...
 * 1.在一个方法内部，在方法内部通过一个lambda表达式的返回，直接作为外层方法返回。在一个方法内部执行一个方法，内部方法有返回值，内部方法的返回值不能表示外层真正的方法的的返回值，想要把内部方法的返回值作为外部方法的返回值，这就是非局部返回。
 * 2.非局部返回的前提是要求外部的方法是inline。
 * 3.某些情况下，内部方法可以引用外部方法外面的对象或者上下文。
 * crossinline：
 * 被标记的内部方法不允许非局部返回，必须要从外部方法的return语句返回。避免内部方法的返回作为外部方法的返回，会导致程序错乱的问题。
 *
 * flowOf：
 * 文档：
